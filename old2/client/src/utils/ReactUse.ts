...
export function useCookieType<T>(name:string, encoder:Encoder<T>, decoder:Decoder<T>, defaultValue:T|undefined=undefined){
    const [cookie, setCookie] = useCookies([name])
    const new_cookie = cookie[name]
    const new_setCookie = (v:string) => setCookie(name, v)
    const getter = () => decoder(new_cookie)
    const setter = (v:T) => new_setCookie(encoder(v))
    if(new_cookie===undefined && defaultValue!==undefined) setter(defaultValue)
